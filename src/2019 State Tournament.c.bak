...
#pragma config(Motor,  port1,           descore,       tmotorVex393_HBridge, openLoop)
...
void pre_auton()
{
    ...
    pidInit(motorPositionPID, 0.009, 0, 0);
...
autonNearFlagNoPlat()
{
    ...
    // Turn 90 deg (to line up with second cap)
    gyroTurn(90 * turnDirection);

    // Drive up to line up
    driveStraight(1100, true);

    // Turn towards 2nd cap
    gyroTurn(-90 * turnDirection);
...
task usercontrol()
{
    ...
    while (true)
    {
        ...
        //if (vexRT[Btn5D] == 1)
          //  motor[flipR] = -30;
        if (vexRT[Btn5U] == 1)
            startTask(descoreCap);
        if (vexRT[Btn5D] == 1)
            startTask(undescoreCap);
    }
}
