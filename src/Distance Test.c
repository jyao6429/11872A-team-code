...
goToPosition(long targetPosition, int maxSpeed)
{
    ...
    while (!bAtTarget)
    {
        long currentPositionR = getMotorEncoder(right1);
        long currentPositionL = getMotorEncoder(left1);
        ...
        float driveOutL = driveOutR + pidCalculate(keepStraightPID, currentPositionR, currentPositionL) * 30;
        ...
        motor[right1] = driveOutR;
        motor[left1] = driveOutL;
